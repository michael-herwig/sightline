...
export const jbGear = (it) =>
  it && Array.isArray(it.gear) ? it.gear.filter((g) => JUNCTIONS[g.model]) : [];

export const jbPower = (it) => jbGear(it).some((g) => JUNCTIONS[g.model].power);

export const jbSfp = (it) => jbGear(it).some((g) => JUNCTIONS[g.model].sfp);

export const jbPoe = (it) =>
...
export const jbDraw = (it) =>
  jbGear(it).reduce((a, g) => {
    const m = JUNCTIONS[g.model];
    return a + (powerIn(m) === "poe" ? (m.poeDraw != null ? m.poeDraw : 15) * g.n : 0);
  }, 0);

export const gearPrice = (it) => jbGear(it).reduce((a, g) => a + JUNCTIONS[g.model].price * g.n, 0);

export const jbPrice = (it) => ((JUNCTIONS[it.model] || {}).price || 0) + gearPrice(it);

// A splice box is allowed to split — then the cable balance at the point is no longer a finding.
export const jbSplice = (it) => jbGear(it).some((g) => g.model === "splice");
...
export const sfpPorts = (it) =>
  jbGear(it).reduce((a, g) => {
    const m = JUNCTIONS[g.model];
    return a + (m.sfpPorts != null ? m.sfpPorts : m.sfp ? 1 : 0) * g.n;
  }, 0);

export const gearNames = (it) =>
  jbGear(it)
...
export const ROUTERS = INFRA.filter((i) => i.role === "router");
...
export const hubPower = (it) => !!hubRouter() || jbPower(it);

export const hubSfp = (it) => {
  const r = hubRouter();
  return !!(r && r.sfp) || jbSfp(it);
};

export const hubPoe = (it) => ((hubRouter() || {}).poe || 0) + jbPoe(it);

export const hubPorts = (it) => ((hubRouter() || {}).ports || 0) + jbPorts(it);

// The UCG has two SFP+, but only one can be switched to LAN — the other stays WAN.
export const hubSfpPorts = (it) => ((hubRouter() || {}).sfp ? 1 : 0) + sfpPorts(it);
...
export function jbGearGroups(jbs) {
  const g = {};
  jbs.forEach((i) =>
    jbGear(i).forEach((x) => {
