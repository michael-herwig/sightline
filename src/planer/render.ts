...
export function jumpView(apply) {
...
export function zoomAt(f, cx, cy) {
...
export function refreshOffsets() {
  ...
  gCond.querySelectorAll("g.conduit[data-id]").forEach((g) => {
    ...
    g.querySelectorAll("path[data-off]").forEach((p) =>
      p.setAttribute("d", dOf(offsetPath(pts, +p.dataset.off * k * LOOK.line))),
    );
    if (
...
function drawHandles() {
  ...
  const c = state.conduits.find((x) => x.id === sel.id);
  if (!c) return;
...
function drawCluster(grp) {
  ...
  const worst = grp.map((i) => (links().status.get(i.id) || {}).g).find((x) => x === "err")
    ? "err"
    : null;
  if (worst) {
...
export function refreshConduit(c) {
