...
// What the two matchers need off a filter badge out of FACETS & co.
type Facet = { id: string; test: (m: Model) => boolean };

// ---------- Build: catalog with search and filter badges ----------
function matchesQuery(m: Model, key: string, q: string) {
  if (!q) return true;
...
function matchesFacets(m: Model, set: Facet[]) {
...
function catCard(card: HTMLElement, kind: string, key: string, onInfo?: () => void) {
  const row = h(`<div class="itemrow" data-hover="${kind}:${esc(key)}"></div>`)
...
function catButton(kind: string, key: string, m: Model, extra: string) {
  return h(`<button class="item" data-kind="${kind}" data-key="${esc(key)}">
    ...
    ${m.tags ? `<span class="tags">${(tx(m.tags) || []).map((x: string) => `<span class="tag ${/kein PoE|no PoE|WLAN|Wi-Fi|Strom nötig|needs power|Erdarbeit|earthwork|nicht mehr zugänglich|not reachable|keine Glasfaser|no fibre|Fachbetrieb|specialist|kein NVR|no NVR/.test(x) ? "w" : ""}">${esc(x)}</span>`).join("")}</span>` : ""}
...
export function syncPressed(root: HTMLElement) {
  const cur: Record<string, string | false> = {
  ...
  root.querySelectorAll<HTMLElement>("[data-kind][data-key]").forEach((b) => {
...
function fillList(id: string, sig: string, keys: string[], make: (k: string) => Node) {
  const box = $(id);
  if (!box) return;
...
export function addGear(it: Item, key: string) {
  it.gear = it.gear || [];
...
function fillJbs() {
  ...
  fillList("cat-jbs", catSig(), keys, (k: string) => {
    if (k[0] === "#")
      return h(`<h3 class="subhead">${esc(t("cat.jb." + k.slice(1)))}</h3>`)
        .firstElementChild as HTMLElement;
    const j = JUNCTIONS[k];
...
function fillConds() {
  ...
  fillList("cat-cond", catSig(), keys, (k: string) => {
    if (k[0] === "#")
      return h(`<h3 class="subhead">${esc(t("cat." + k.slice(1)))}</h3>`)
        .firstElementChild as HTMLElement;
    const ct = CONDUITS[k];
...
export const FILL: Record<string, () => void> = {
  cam: fillCams,
  ap: fillAps,
  jb: fillJbs,
  gear: fillGear,
